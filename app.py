...
with st.sidebar:
    ...
    if st.button("Clear Memory"):
        for key in ["messages", "full_text", "vector_store", "mode"]:
...
if uploaded_files and api_key and not st.session_state.mode:
    with st.spinner("Analyzing Documents (Anti-Timeout Mode)..."):
        try:
            all_text = ""
            total_pages = 0
            ...
            if total_pages <= 5:
                ...
                st.sidebar.success(f"Mode: Direct ({total_pages} pages)")
            else:
                st.session_state.mode = "RAG"
                st.sidebar.info(f"Mode: RAG ({total_pages} pages)")
                
                text_splitter = RecursiveCharacterTextSplitter(chunk_size=800, chunk_overlap=100)
                chunks = text_splitter.split_text(all_text)
                
                # --- CRITICAL FIX: TRANSPORT & TIMEOUT ---
...
if prompt := st.chat_input("Ask about your documents..."):
    if not api_key: st.error("Please enter API Key.")
    elif not uploaded_files: st.info("Upload PDFs first.")
    else:
        ...
        with st.chat_message("assistant"):
            try:
                ...
                context = st.session_state.full_text if st.session_state.mode == "DIRECT" else \
                          "\n\n".join([d.page_content for d in st.session_state.vector_store.similarity_search(prompt, k=5)])
